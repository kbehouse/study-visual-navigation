...
if __name__ == '__main__':
  ...
  print("checkpoint={}, checkpoint.model_checkpoint_path={}".format(checkpoint, checkpoint.model_checkpoint_path))
  ...
  for scene_scope in scene_scopes:
    ...
    for task_scope in list_of_tasks[scene_scope]:
      ...
      task_scope_dir = log_dir + 'task_scope-' + str(task_scope) +'/'
      recreate_dir(task_scope_dir)
      # ['navigation', bathroom_02, '26']
      ...
      for i_episode in range(NUM_EVAL_EPISODES):
        ...
        while not terminal:
          ...
          action = sample_action(pi_values) # roulette selection
          env.step(action)
          env.update()

          # env.s_t.shape=(2048, 4), env.target.shape=(2048, 4)
          # scopes=['navigation', 'bathroom_02', '26']  (only one example)
          # pi_values=[9.6079403e-01 4.6466477e-03 3.3979964e-02 5.7935994e-04]  sum is 1.0 (only one example)
          # action=0 , 1, 2, 3

          #print("env.s_t.shape={}, env.target.shape={}, scopes={}".format(env.s_t.shape, env.target.shape, scopes))
          #print("pi_values={}".format(pi_values))
          ...
          # print("env.x={},env.z={},env.r={}".format(env.x, env.z, env.r))
          save_path = task_scope_dir + "[#{}_step{}_action{}]_x{:.2f}_z{:.2f}_r{:.2f}.png".format( i_episode, ep_t, action, env.x, env.z, env.r) 
          
          cv2.imwrite(save_path, env.observation)
          
          # viewer = SimpleImageViewer()
          # viewer.imshow(env.observation)
          # time.sleep(0.5)

        # print(' env.target= {} Finish, ep_collision ={}, ep_reward={}, ep_t={}'.format( \
        #    env.target, ep_collision, ep_reward, ep_t \
        # ))
